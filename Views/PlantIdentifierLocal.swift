...
/// Loads a compiled .mlmodel (MyPlantClassifier.mlmodel or whatever you name it)
/// from the app bundle at runtime and runs VNCoreMLRequest on UIImages.
/// This avoids relying on a generated Swift model class.
final class PlantIdentifierLocal {
    private let vnModel: VNCoreMLModel

    /// modelFileName: name (without extension) of the compiled model you added to the app bundle.
    /// Default: "MyPlantClassifier" — change this if you named your model differently.
    init?(modelFileName: String = "MyPlantClassifier") {
        // Look for a compiled model in the app bundle (.mlmodelc) or the .mlmodel
        // We prefer the compiled version (mlmodelc) but MLModel(contentsOf:) works for both.
