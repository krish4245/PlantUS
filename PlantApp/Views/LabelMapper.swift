...
/// Map model labels to app display names / PlantData keys.
/// Edit the dictionary below if your model labels differ.
struct LabelMapper {
    /// Key: raw label returned by the model (case-insensitive)
    /// Value: the normalized app name used in PlantData (exact match to PlantData.browse[].name)
    ...
    /// Normalize raw label -> app name (nil if not mappable)
